...
- 100% CUDA конвейер с поддержкой spawn multiprocessing.
- 50/100 Гц нотч-фильтры.
- Векторный расчет частоты Теты без torch.unwrap.
- 32 Гамма-слота PAC (Working Memory 2.0).
"""
    ...
    def run(self):
        ...
        while self.shm['is_running'].value:
            ...
            with torch.inference_mode():
                ...
                mean_th_phasor = torch.mean(P_theta, dim=1, keepdim=True) # [4, 1, BUF_SIZE]
                phi_theta = torch.angle(mean_th_phasor).unsqueeze(1)       # [4, 1, 1, BUF_SIZE]
                
                # Синхронизация Тета
                sync_r = float(torch.mean(torch.abs(mean_th_phasor[0])).item())
                self.shm['theta_sync'].value = sync_r

                # --- РАСЧЕТ ЖИВОЙ ЧАСТОТЫ ТЕТЫ БЕЗ torch.unwrap ---
